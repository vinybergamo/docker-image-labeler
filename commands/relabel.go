...
func (c *RelabelCommand) Run(args []string) int {
	...
	if err != nil {
		c.Ui.Error(fmt.Sprintf("Failed to create a docker client: %s", err.Error()))
		return 1
	}
	...
	alternateTagsLabel := "com.vinybergamo.vinybergamo-image-labeler/alternate-tags"
	...
	if err != nil {
		c.Ui.Error(fmt.Sprintf("Failed added labels: %s", err.Error()))
		return 1
	}
	...
	if dockerClient.ImageRemove(context.Background(), originalImageID.String(), options); err != nil {
